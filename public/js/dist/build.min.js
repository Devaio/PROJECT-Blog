...
if(deprecate){global.moment=function(){if(!warned&&console&&console.warn){warned=true;console.warn("Accessing Moment through the global scope is "+"deprecated, and will be removed in an upcoming "+"release.");}
...
return moment;});}else{makeGlobal();}}).call(this);},{}],2:[function(require,module,exports){(function(){function patchXHR(fnName,newFn){window.XMLHttpRequest.prototype[fnName]=newFn(window.XMLHttpRequest.prototype[fnName]);}
...
if(!config.disableProgress){config.headers.__setXHR_=function(){return function(xhr){if(!xhr)return;config.__XHR=xhr;if(config.xhrFn)config.xhrFn(xhr);xhr.upload.addEventListener('progress',function(e){e.config=config;notifyProgress(getNotifyEvent(e));},false);xhr.upload.addEventListener('load',function(e){if(e.lengthComputable){e.config=config;notifyProgress(getNotifyEvent(e));}},false);};};}
...
upload.updateModel=function(ngModel,attr,scope,fileChange,files,evt,noDelay){function update(files,invalidFiles,newFiles,dupFiles,isSingleModel){var file=files&&files.length?files[0]:null;if(ngModel){upload.applyModelValidation(ngModel,files);ngModel.$setViewValue(isSingleModel?file:files);}
...
upload.validate(newFiles,ngModel,attr,scope).then(function(){if(noDelay){update(files,[],newFiles,dupFiles,isSingleModel);}else{var options=upload.attrGetter('ngModelOptions',attr,scope);if((!options||!options.allowInvalid)&&!validateAfterResize){separateInvalids();}
...
upload.validate(files,ngModel,attr,scope).then(function(){upload.applyModelValidation(ngModel,files);});}});}};function markModelAsDirty(ngModel,files){if(files!=null&&!ngModel.$dirty){if(ngModel.$setDirty){ngModel.$setDirty();}else{ngModel.$dirty=true;}}}
...
return val;};upload.validate=function(files,ngModel,attr,scope){ngModel=ngModel||{};ngModel.$ngfValidations=ngModel.$ngfValidations||[];angular.forEach(ngModel.$ngfValidations,function(v){v.valid=true;});var attrGetter=function(name,params){return upload.attrGetter(name,attr,scope,params);};if(files==null||files.length===0){return upload.emptyPromise(ngModel);}
...
var promises=[upload.emptyPromise()];var items=evt.dataTransfer.items;if(items&&items.length>0&&$location.protocol()!=='file'){for(var i=0;i<items.length;i++){if(items[i].webkitGetAsEntry&&items[i].webkitGetAsEntry()&&items[i].webkitGetAsEntry().isDirectory){var entry=items[i].webkitGetAsEntry();if(entry.isDirectory&&!allowDir){continue;}
if(entry!=null){promises.push(traverseFileTree(entry));}}else{var f=items[i].getAsFile();if(f!=null){files.push(f);totalSize+=f.size;}}
if(files.length>maxFiles||totalSize>maxTotalSize||(!multiple&&files.length>0))break;}}else{var fileList=evt.dataTransfer.files;if(fileList!=null){for(var j=0;j<fileList.length;j++){var file=fileList.item(j);if(file.type||file.size>0){files.push(file);totalSize+=file.size;}
if(files.length>maxFiles||totalSize>maxTotalSize||(!multiple&&files.length>0))break;}}}
var defer=$q.defer();$q.all(promises).then(function(){if(!multiple){var i=0;while(files.length&&files[i].type==='directory')i++;defer.resolve([files[i]]);}else{defer.resolve(files);}},function(e){defer.reject(e);});return defer.promise;}}
...
chr1=chr2=chr3='';enc1=enc2=enc3=enc4='';}while(i<input.length);return buf;};return ExifRestorer.restore(orig,resized);};return upload;}]);},{}],3:[function(require,module,exports){require('./dist/ng-file-upload-all');module.exports='ngFileUpload';},{"./dist/ng-file-upload-all":2}],4:[function(require,module,exports){require('ng-file-upload');angular.module('BlogApp',['ngMaterial','ngAnimate','ui.tinymce','ui.router','ngResource','ngSanitize','ngFileUpload','updateMeta','angular-pinterest','angulartics','angulartics.google.analytics']).config(function($mdThemingProvider){return $mdThemingProvider.theme('default').primaryPalette('grey',{"default":'300'}).accentPalette('deep-purple').warnPalette('light-green');});require('./services/authService');require('./directives/nav');require('./factories/post-tag');require('./factories/social');require('./controllers/main/home');require('./controllers/main/about');require('./controllers/main/post');require('./controllers/main/tag');require('./controllers/admin/adminLogin');require('./controllers/admin/adminDashboard');require('./controllers/admin/adminAddPost');require('./controllers/admin/adminUpdatePost');require('./controllers/admin/adminModerateComments');require('./routes');},{"./controllers/admin/adminAddPost":5,"./controllers/admin/adminDashboard":6,"./controllers/admin/adminLogin":7,"./controllers/admin/adminModerateComments":8,"./controllers/admin/adminUpdatePost":9,"./controllers/main/about":10,"./controllers/main/home":11,"./controllers/main/post":12,"./controllers/main/tag":13,"./directives/nav":14,"./factories/post-tag":15,"./factories/social":16,"./routes":17,"./services/authService":18,"ng-file-upload":3}],5:[function(require,module,exports){angular.module('BlogApp').controller('adminAddPost',['$scope','$location','$http','$sce','$timeout','authService','Upload',function($scope,$location,$http,$sce,$timeout,authService,Upload){tinymce.PluginManager.add('htmllink',function(editor,url){return editor.addButton('htmllinkbtn',{text:'HTML Link',icon:false,onclick:function(){return editor.windowManager.open({title:'Please input text',body:[{type:'textbox',name:'link',label:'Paste Link'}],onsubmit:function(e){return editor.insertContent(e.data.link);}});}});});$scope.navheight='small';$scope.loading=false;$scope.newPost={tags:[],createdAt:new Date()};$scope.$sce=$sce;$scope.resizeCheck=function(file,width,height){return width>1600||height>1200;};$scope.tinymceOptions={onChange:function(e){},inline:false,plugins:'advlist autolink hr link image lists charmap print preview fontselect fontsizeselect htmllink',skin:'lightgray',theme:'modern',browser_spellcheck:true,font_formats:"GeoSans=GeoSans",paste_as_text:true,height:600,browser_spellcheck:true,fontsize_formats:"8pt 9pt 10pt 11pt 12pt 14pt 18pt 26pt 36pt",default_link_target:"_blank",toolbar1:'core',toolbar2:"undo redo pastetext | styleselect | fontselect | fontsizeselect | bold italic |  alignleft aligncenter alignright alignjustify |  bullist numlist outdent indent blockquote hr | link image | htmllinkbtn",width:740};$scope.submit=function(){var uploader;uploader=Upload.upload({url:'/api/media',data:{files:$scope.files}});return uploader.then(function(returnData){return returnData.data.data.forEach(function(file,index){return $scope.files[index].url=file.url;});});};$scope.copyText=function(event){event.target.select();return document.execCommand('copy');};return authService(function(stuff){$scope.submitPost=function(){var _ref,_ref1,_ref2,_ref3,_ref4,_ref5;$scope.postError='';if(((_ref=$scope.newPost)!=null?(_ref1=_ref.content)!=null?_ref1.length:void 0:void 0)&&((_ref2=$scope.newPost)!=null?_ref2.preview:void 0)&&((_ref3=$scope.newPost)!=null?_ref3.coverImg:void 0)&&((_ref4=$scope.newPost)!=null?(_ref5=_ref4.tags)!=null?_ref5.length:void 0:void 0)){$scope.loading=true;console.log($scope.newPost);$scope.newPost.createdAt=$scope.newPost.createdAt.getTime();return $http.post('/api/posts',$scope.newPost).then(function(returnData){console.log(returnData);$scope.loading=false;if(returnData.data._id){return $timeout(function(){return $location.url('/posts/'+returnData.data.slug);});}else{return $scope.postError=returnData.data;}});}else{return $scope.postError='Make sure everything is filled out!';}};return $scope.submitDraft=function(){$scope.loading=true;console.log($scope.newPost);$scope.newPost.deleted=true;$scope.newPost.createdAt=$scope.newPost.createdAt.getTime();return $http.post('/api/posts',$scope.newPost).then(function(returnData){console.log(returnData);$scope.loading=false;if(returnData.data._id){return alert('Draft Saved!');}else{return alert('Bad things happened!'+returnData.data);}});};});}]);},{}],6:[function(require,module,exports){angular.module('BlogApp').controller('adminDashboard',['$scope','$location','authService',function($scope,$location,authService){$scope.navheight='small';return authService(function(stuff){$scope.addPost=function(){return $location.url('/admin/addpost');};$scope.updatePost=function(){return $location.url('/admin/updatepost');};return $scope.moderateComments=function(){return $location.url('/admin/moderatecomments');};});}]);},{}],7:[function(require,module,exports){angular.module('BlogApp').controller('adminLogin',['$scope','$http','$location',function($scope,$http,$location){$scope.navheight='small';$scope.loading=false;return $scope.loginFire=function(){$scope.loading=true;$scope.loginError='';return $http.post('/admin/login',$scope.adminUser).then(function(returnData){$scope.loading=false;if(returnData.data.error){return $scope.loginError=returnData.data.error;}else{return $location.url('/admin/dashboard');}});};}]);},{}],8:[function(require,module,exports){var moment;moment=require('moment');angular.module('BlogApp').controller('adminModerateComments',['$scope','$location','$http','$timeout','authService','$sce','$filter',function($scope,$location,$http,$timeout,authService,$sce,$filter){$scope.navheight='small';$scope.$sce=$sce;$scope.moment=moment;$scope.linky=$filter('linky');return authService(function(stuff){$http.get('/api/comments').then(function(returnData){return $scope.commentList=returnData.data;});$scope.urlCheck=function(url){if(!url.match('http://')||!url.match('https://')){return'http://'+url;}else{return url;}};$scope.linker=function(content){if(content.match(/<[^>]+>/gm)){return $sce.trustAsHtml(content);}else{return $scope.linky(content);}};$scope.approveComment=function(comment){comment.approved=true;return $http.post('/api/comments/'+comment._id,comment);};return $scope.deleteComment=function(comment,index,post){$http["delete"]('/api/comments/'+comment._id);return $scope.commentList[post].splice(index,1);};});}]);},{"moment":1}],9:[function(require,module,exports){var moment;moment=require('moment');angular.module('BlogApp').controller('adminUpdatePost',['$scope','$location','$http','$sce','$timeout','authService','$stateParams','postTagFactory','Upload',function($scope,$location,$http,$sce,$timeout,authService,$stateParams,postTagFactory,Upload){tinymce.PluginManager.add('htmllink',function(editor,url){return editor.addButton('htmllinkbtn',{text:'HTML Link',icon:false,onclick:function(){return editor.windowManager.open({title:'Please input text',body:[{type:'textbox',name:'link',label:'Paste Link'}],onsubmit:function(e){return editor.insertContent(e.data.link);}});}});});$scope.navheight='small';$scope.loading=false;postTagFactory.postModel.query({all:10000,deleted:true},function(data){$scope.posts=data;return $scope.posts.forEach(function(post){return post.tags=post.tags||[];});});$scope.$sce=$sce;console.log($scope);$scope.transformDate=function(){return $scope.selectedPost.createdAt=new Date($scope.selectedPost.createdAt);};$scope.resizeCheck=function(file,width,height){return width>1600||height>1200;};$scope.submit=function(){var uploader;uploader=Upload.upload({url:'/api/media',data:{files:$scope.files}});return uploader.then(function(returnData){return returnData.data.data.forEach(function(file,index){return $scope.files[index].url=file.url;});});};$scope.tinymceOptions={onChange:function(e){},inline:false,plugins:'advlist autolink hr link image lists charmap print preview fontselect fontsizeselect htmllink',skin:'lightgray',theme:'modern',browser_spellcheck:true,paste_as_text:true,height:600,browser_spellcheck:true,font_formats:"GeoSans=GeoSans",fontsize_formats:"8pt 9pt 10pt 11pt 12pt 14pt 18pt 26pt 36pt",default_link_target:"_blank",toolbar1:'core',toolbar2:"undo redo pastetext | styleselect | fontselect | fontsizeselect | bold italic |  alignleft aligncenter alignright alignjustify |  bullist numlist outdent indent blockquote hr | link image | htmllinkbtn",width:740};$scope.copyText=function(event){event.target.select();return document.execCommand('copy');};return authService(function(stuff){return $scope.updatePost=function(){var _ref,_ref1;if((_ref=$scope.selectedPost)!=null?(_ref1=_ref.content)!=null?_ref1.length:void 0:void 0){$scope.loading=true;return $scope.selectedPost.$save(function(){$scope.loading=false;window.alert('Done!');window.location.reload();return true;});}};});}]);},{"moment":1}],10:[function(require,module,exports){angular.module('BlogApp').controller('aboutCont',['$scope',function($scope){return $scope.navheight='small';}]);},{}],11:[function(require,module,exports){angular.module('BlogApp').controller('homeCont',['$scope','$sce','postTagFactory','$stateParams','$location','socialFactory',function($scope,$sce,postTagFactory,$stateParams,$location,socialFactory){window.scrollTo(0,0);$scope.navheight='large';console.log($scope,socialFactory);$scope.socialData=socialFactory.socialData;$scope.posts=postTagFactory.posts($stateParams.pageNum);$scope.nextPage=parseInt($stateParams.pageNum||1)+1;$scope.showNextPage=true;$scope.socialFactory=socialFactory;postTagFactory.random(function(data){return $scope.morePosts=data;});if(parseInt($stateParams.pageNum||1)<=1){$location.url('/');}
$scope.$watch('posts.length',function(){if($scope.posts.length<10){return $scope.showNextPage=null;}else{$scope.nextPage=parseInt($stateParams.pageNum||1)+1;return $scope.showNextPage=true;}});$scope.$sce=$sce;return $scope.commentLengthChecker=function(post){var totalComments;totalComments=post.comments.reduce(function(rt,cn){var countCurrent;countCurrent=cn.approved?1:0;return rt+countCurrent;},0);return totalComments;};}]);},{}],12:[function(require,module,exports){var moment;moment=require('moment');angular.module('BlogApp').controller('postCont',['$scope','$sce','$stateParams','postTagFactory','$location','$window','$http','$filter','$timeout','socialFactory',function($scope,$sce,$stateParams,postTagFactory,$location,$window,$http,$filter,$timeout,socialFactory,authService){$scope.navheight='small';$scope.moment=moment;$scope.newComment={};$scope.url=$location.$$absUrl;$scope.linky=$filter('linky');$scope.socialData=socialFactory.socialData;$scope.posts=postTagFactory.posts($stateParams.pageNum);$scope.socialFactory=socialFactory;postTagFactory.random(function(data){return $scope.morePosts=data;});if($location.$$hash==='comments'){$scope.commentBox=angular.element('#comments');window.scrollTo(0,$scope.commentBox.offsetTop);}
$scope.post=postTagFactory.postModel.get({id:window.location.pathname.split('/')[2]});$scope.$sce=$sce;$scope.linker=function(content){if(content.match(/<[^>]+>/gm)){return $sce.trustAsHtml(content);}else{return $scope.linky(content);}};$scope.urlCheck=function(url){if(!url.match('http://')||!url.match('https://')){return'http://'+url;}else{return url;}};$scope.pinIt=function(){var img;img=$scope.post.pinImg;return'https://pinterest.com/pin/create/bookmarklet/?url='+$scope.url+'&media='+encodeURIComponent(img)+'&description=theviewfromhere.is%20%7C%7C%20'+encodeURIComponent($scope.post.title);};$scope.submitComment=function(form){if(!$scope.newComment.name||!$scope.newComment.content||!$scope.newComment.content.length){return $scope.errorMsg='Please fill out the form!';}else{return $http.post('/api/comments/create/'+$scope.post._id,$scope.newComment).then(function(returnData){$scope.newComment={name:''};$scope.errorMsg='';$scope.successMsg='Thanks!  Your comment is awaiting moderation.';form.$setPristine();return form.$setUntouched();});}};$scope.subCommentForm=function(comment){return comment.showSubCommentForm=!comment.showSubCommentForm;};$scope.submitSubComment=function(parentComment,subComment,form){var parentCommentCopy;parentCommentCopy=angular.copy(parentComment);if(!subComment.name||!subComment.content||!subComment.content.length){return subComment.errorMsg='Please fill out the form!';}else{subComment.isSubComment=true;subComment.parentComment=parentCommentCopy;return $http.post('/api/comments/create/'+$scope.post._id,subComment).then(function(returnData){subComment.errorMsg='';subComment.successMsg='Thanks!  Your comment is awaiting moderation.';subComment.name='';subComment.email='';subComment.website='';subComment.content='';form.$setPristine();form.$setUntouched();$timeout(function(){return parentComment.showSubCommentForm=false;},2200);parentCommentCopy.subComments=parentCommentCopy.subComments||[];parentCommentCopy.subComments.push(returnData.data);return $http.post('/api/comments/'+parentCommentCopy._id,parentCommentCopy);});}};return $http.get('/api/me').then(function(returnData){if(returnData.data.user){$scope.loggedInUser=returnData.data.user;$scope.loggedInUser.website='http://theviewfromhere.is';return $scope.loggedInUser.name='Clarissa';}});}]);},{"moment":1}],13:[function(require,module,exports){angular.module('BlogApp').controller('tagCont',['$scope','$sce','$stateParams','postTagFactory','$location','socialFactory',function($scope,$sce,$stateParams,postTagFactory,$location,socialFactory){$scope.navheight='small';window.scrollTo(0,0);$scope.tag=postTagFactory.tagModel.get({name:$stateParams.name});$scope.$sce=$sce;$scope.socialFactory=socialFactory;$scope.posts=postTagFactory.postModel.query({tag:$stateParams.name,page:$stateParams.pageNum});$scope.nextPage=parseInt($stateParams.pageNum||1)+1;$scope.showNextPage=true;postTagFactory.random(function(data){return $scope.morePosts=data;});if(parseInt($stateParams.pageNum)<=1){$location.url("/tags/"+$stateParams.name);}
$scope.$watch('posts.length',function(){if($scope.posts.length<10){return $scope.showNextPage=null;}else{$scope.nextPage=parseInt($stateParams.pageNum||1)+1;return $scope.showNextPage=true;}});return $scope.commentLengthChecker=function(post){var totalComments;totalComments=post.comments.reduce(function(rt,cn){var countCurrent;countCurrent=cn.approved?1:0;return rt+countCurrent;},0);return totalComments;};}]);},{}],14:[function(require,module,exports){angular.module('BlogApp').directive('nav',function(){return{restrict:'E',templateUrl:'/public/templates/directives/nav.html',scope:{navheight:'='},controller:function($scope,$mdSidenav){$scope.toggleNav=function(){return $mdSidenav('right').toggle();};return $scope.toggleSub=function(){return $scope.showSubscribe=!$scope.showSubscribe;};}};});},{}],15:[function(require,module,exports){angular.module('BlogApp').factory('postTagFactory',['$resource','$stateParams','$http',function($resource,$stateParams,$http){var postModel,tagModel;postModel=$resource('/api/posts/:id',{id:'@_id'});tagModel=$resource('/api/tags/:id',{id:'@_id'});return{postModel:postModel,tagModel:tagModel,posts:function(page){return postModel.query({page:page});},random:function(cb){return $http.get('/api/posts/random?num=5').then(function(returnData){return cb(returnData.data);});}};}]);},{}],16:[function(require,module,exports){angular.module('BlogApp').factory('socialFactory',['$resource','$stateParams','$http','$timeout',function($resource,$stateParams,$http,$timeout){var socialFactory;socialFactory={};socialFactory.socialData={};$http.get('https://api.pinterest.com/v1/me/pins/?access_token=Ai5IOiQg8IAUUVqV3M_9kC9q5CP8FXK5CaTpzTxDBj_v88AsKAE-wDAAAAfQRXx0UFkgb_EAAAAA&fields=id%2Cnote%2Curl%2Cimage').then(function(returnData){return socialFactory.socialData.sidebarPins=returnData.data.data.slice(0,12);});socialFactory.subscribe=function(sub){sub.loading=true;return $http({url:'/api/subscribe',method:"POST",data:{email:sub.email}}).then(function(resp){sub.loading=false;socialFactory.subscribeSuccess='Thanks for signing up!';sub.email='';return $timeout(function(){return socialFactory.subscribeSuccess='';},3000);});};return socialFactory;}]);},{}],17:[function(require,module,exports){angular.module('BlogApp').config(['$stateProvider','$urlRouterProvider','$locationProvider','$compileProvider',function($stateProvider,$urlRouterProvider,$locationProvider,$compileProvider){$compileProvider.debugInfoEnabled(false);$locationProvider.html5Mode(true);$urlRouterProvider.otherwise('/');return $stateProvider.state('home',{url:'/',controller:'homeCont',templateUrl:'/public/templates/main/home.html'}).state('pages',{url:'/page/:pageNum',controller:'homeCont',templateUrl:'/public/templates/main/home.html'}).state('about',{url:'/about',controller:'aboutCont',templateUrl:'/public/templates/main/about.html'}).state('post',{url:'/posts/:id',controller:'postCont'}).state('postEdit',{url:'/editing/:id',controller:'postCont'}).state('tag',{url:'/tags/:name',controller:'tagCont',templateUrl:'/public/templates/main/tag.html'}).state('tagPage',{url:'/tags/:name/:pageNum',controller:'tagCont',templateUrl:'/public/templates/main/tag.html'}).state('adminLogin',{url:'/admin/login',controller:'adminLogin',templateUrl:'/public/templates/admin/login.html'}).state('adminDashboard',{url:'/admin/dashboard',controller:'adminDashboard',templateUrl:'/public/templates/admin/dashboard.html'}).state('adminAddPost',{url:'/admin/addpost',controller:'adminAddPost',templateUrl:'/public/templates/admin/addpost.html'}).state('adminUpdatePost',{url:'/admin/updatepost',controller:'adminUpdatePost',templateUrl:'/public/templates/admin/updatepost.html'}).state('adminModerateComments',{url:'/admin/moderatecomments',controller:'adminModerateComments',templateUrl:'/public/templates/admin/moderateComments.html'}).state('contact',{url:'/contact',templateUrl:'/public/templates/main/contact.html'}).state('disclosure',{url:'/disclosure',templateUrl:'/public/templates/main/disclosure.html'});}]).run(['$rootScope',function($rootScope){return $rootScope.$on('$stateChangeSuccess',function(event,toState,toParams,fromState,fromParams){if(toParams.id&&fromState.name){return location.reload();}});}]);},{}],18:[function(require,module,exports){angular.module('BlogApp').service('authService',['$http','$location',function($http,$location){return this.authCheck=function(cb){return $http.get('/api/me').then(function(returnData){if(returnData.data.user){return cb(returnData.data.user);}else{return $location.url('/');}});};}]);},{}]},{},[4])