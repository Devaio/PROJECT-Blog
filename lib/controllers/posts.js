...
Posts = (function(_super) {
    ...
    if (req.query.tag) {
      return Tag.findOne({
        ...
        if (tag) {
          ...
          return Post.find(q).sort('-createdAt').limit(postLimit).skip(pageSkip).populate('tags comments').then(function(err, posts) {
            var post, _i, _len;
            if (posts != null) {
  ...
  Posts.prototype.random = function(req, res) {
    return Post.count().then(function(err, count) {
      return Post.find().then(function(err, posts) {
        var shufflePosts;
  ...
  Posts.prototype.getDeleted = function(req, res) {
    return Post.find({
      deleted: true
    }).sort('-createdAt').then(function(err, posts) {
      return res.send(posts);
    });
  };
