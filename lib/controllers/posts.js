...
var Posts = (function (_super) {
    ...
    Posts.prototype.getPageHidden = function (req, res) {
        console.log('GETTING HIDDEN', req.params);
        posts_1.Post.findOne({ _id: req.params.id, deleted: true }).populate('tags').populate({
            path: 'comments',
            populate: {
                path: 'subComments',
                model: 'Comment'
            ...
            if (post) {
                ...
                posts_1.Post.findOne({ slug: req.params.id, deleted: true }).populate('tags').populate({
                    path: 'comments',
                    populate: {
                        path: 'subComments',
                        model: 'Comment'
