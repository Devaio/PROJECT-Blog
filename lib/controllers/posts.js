    ...
    if (req.query.tag) {
      return Tag.findOne({
        ...
        if (tag) {
          ...
          return Post.find(q).sort('-createdAt').limit(postLimit).skip(pageSkip).populate('tags').exec(function(err, posts) {
            var post, _i, _len;
            if (posts != null) {
      ...
      if (req.params.id) {
        return Post.findOne({
          _id: req.params.id,
          deleted: false
        }).sort('-createdAt').skip(pageSkip).populate('tags').exec(function(err, post) {
          if (post) {
            toReadableDate(post);
          }
